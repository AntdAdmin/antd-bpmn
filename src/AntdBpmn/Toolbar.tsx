...
const Toolbar: React.FC<{
    modeler: MutableRefObject<BpmnModeler>,
}> = ({modeler}) => {
    ...
    useEffect(() => {
        if (modeler.current) {
            const eventBus: any = modeler.current.get('eventBus');
            eventBus.on('selection.changed', (e: { newSelection: Array<any> }) => {
    ...
    function align(type: "left" | "right" | "center" | "top" | "bottom" | "middle") {
        if (selectItemsRef.current && selectItemsRef.current.length > 1) {
            (modeler.current.get("alignElements") as any).trigger(selectItemsRef.current, type);
        } else {
    ...
    function zoomIn() {
        ...
        (modeler.current.get("canvas") as any).zoom(currentZoomValueRef.current, "auto")
    }
     ...
     */
    function undo() {
        (modeler.current.get("commandStack") as any).undo()
    }
     ...
     */
    function redo() {
        (modeler.current.get("commandStack") as any).redo()
    }
    ...
    return (
        <>
            ...
            <Space>
                <Button type="primary">保存（Ctrl+s）</Button>
